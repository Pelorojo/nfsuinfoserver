...
SERVERS_FILE = f"{nfsuserver_path}/infoservers.csv"
Path(SERVERS_FILE).touch()
...
def handle_registration(client_socket, address, data):
    try:
        ...
        print(f"Error processing registration from {address}: {e}")
    finally:
        client_socket.close()
...
def main():
    ...
    try:
        ...
        central_registry_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        ...
        print(f"Failed to bind to {CENTRAL_REGISTRY_HOST}:{CENTRAL_REGISTRY_PORT}: {e}")
    
    finally:
        central_registry_socket.close()
        print("Central registry server closed")
